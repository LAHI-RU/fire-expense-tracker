...
export default function AnalyticsPage() {
  ...
  const totalIncome = analyticsData.financialOverview.find((item: any) => item.type === "incomes")?.total || 0
  const totalExpenses = analyticsData.financialOverview.find((item: any) => item.type === "expenses")?.total || 0
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {analyticsData.projectProfitability.slice(0, 5).map((project: any, index: number) => (
                <div key={project.id} className="flex items-center justify-between p-3 rounded-lg border border-border">
                  ...
                  <div className="text-right">
                    <p
                      className={`text-sm font-semibold ${Number(project.profit) >= 0 ? "text-green-600" : "text-red-600"}`}
                    >
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {analyticsData.employeeSalaries.map((employee: any) => (
              <div key={employee.employee_code} className="p-4 rounded-lg border border-border">
                ...
                <div className="space-y-1 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">${Number(employee.monthly_salary || 0).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Total Paid (12m):</span>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{employee.payment_count || 0}</span>
                  </div>
                </div>
              </div>
            ))}
