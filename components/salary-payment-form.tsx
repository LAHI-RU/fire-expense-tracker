...
export function SalaryPaymentForm({ onSubmit, onCancel, isLoading }: SalaryPaymentFormProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [employeesData, projectsData] = await Promise.all([employeesRes.json(), projectsRes.json()])
  ...
  useEffect(() => {
    const checkDuplicatePayment = async () => {
      if (!formData.employee_id || !formData.payment_date || formData.payment_type !== "monthly_salary") {
        setDuplicateWarning("")
        return
      }
      ...
      try {
        ...
        const monthlyPayments = data.payments?.filter((p: any) => p.payment_type === "monthly_salary") || []

        if (monthlyPayments.length > 0) {
          setDuplicateWarning(
            `Warning: Monthly salary for ${paymentDate.toLocaleString("default", {
              month: "long",
              year: "numeric",
  ...
  useEffect(() => {
    const employee = employees.find((e) => e.id.toString() === formData.employee_id)
    setSelectedEmployee(employee || null)

    if (employee && formData.payment_type === "monthly_salary" && employee.monthly_salary) {
      setFormData((prev) => ({ ...prev, amount: employee.monthly_salary.toString() }))
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({
      ...
      project_id: formData.project_id ? Number.parseInt(formData.project_id) : undefined,
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {duplicateWarning && (
            <Alert className="border-amber-200 bg-amber-50">
              ...
              <AlertDescription className="text-amber-800">{duplicateWarning}</AlertDescription>
            </Alert>
          )}
